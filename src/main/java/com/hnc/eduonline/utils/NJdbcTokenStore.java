 ...
 * @author nhuongnm
 */
public class NJdbcTokenStore implements TokenStore {

    private static final Log LOG = LogFactory.getLog(NJdbcTokenStore.class);
    private static final LogHelper logger = new LogHelper(NJdbcTokenStore.class);
    ...
    private static final String USERNAME_AND_CLIENT_ID_TOKEN_DELETE_STATEMENT = "delete from oauth_access_token where user_name = ? and client_id = ?";
    ...
    private String deleteAccessTokenByUsernameAndClientID = USERNAME_AND_CLIENT_ID_TOKEN_DELETE_STATEMENT;

    private AuthenticationKeyGenerator authenticationKeyGenerator = new DefaultAuthenticationKeyGenerator();

    private final JdbcTemplate jdbcTemplate;

    public NJdbcTokenStore(DataSource dataSource) {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        // remove already key before insert
        
        removeTokenByUserNameAndClientId(authentication);
        //insert new key
    ...
    private void removeTokenByUserNameAndClientId(OAuth2Authentication authentication)
    {
        removeTokenByUserNameAndClientId(authentication.getPrincipal().toString(),authentication.getOAuth2Request().getClientId());
    }
    
    private void removeTokenByUserNameAndClientId(String username, String clientId)
    {
        logger.logInfo("removeTokenByUserNameAndClientId: " + deleteAccessTokenByUsernameAndClientID + ":"+username +"/"+ clientId);
        //String str = String.format(deleteAccessTokenByUsernameAndClientID, username,clientId);        
        //MessageFormat msg = new MessageFormat("delete from oauth_access_token where user_name = {0} and client_id = {1}");
        String str = MessageFormat.format(deleteAccessTokenByUsernameAndClientID,username,clientId);
        logger.logInfo(str);
        
        jdbcTemplate.update(deleteAccessTokenByUsernameAndClientID, username,clientId);
